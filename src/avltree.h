...
{
	private:
		int compare(TYPE* item_0, TYPE* item_1)
		{
			//-1 : item_0  < item_1
			//0  : item_0 == item_1
			//1  : item_0  > item_1
			cout << "compare [" << item_0->word() << "] | [" << item_1->word() << "]\n";
			int
				hash_0 = item_0->hash(),
				hash_1 = item_1->hash();

			if(hash_0 < hash_1) return -1;
			else if(hash_0 > hash_1) return 1;
			else return strcmp(item_0->word(), item_1->word());//the hashes are the same, compare the strings
		}
		...
		Node* findNode(TYPE* val)//int val)
		{
			Node* temp = root;
			while(temp != 0)
			{
				if(compare(val, temp->getData()) == 0)//if(val == temp->getData())
					return temp;

				else if(compare(val, temp->getData()) == -1)//else if(val < temp->getData())
	...
	public:
		...
		AVLTree(TYPE* val)//int val)
		{
			root = new Node(val);
		}

		AVLTree * findSubtree(TYPE* val)//int val)
		{
			Node* target;
			target = findNode(val);
		...
		// Returns: 
		// 		NULL: If addition is successful
		// 		TYPE*: If item already exists
		//
		TYPE* insert(TYPE* val)//int val)
		{
			Node* added_node;
			if(root == 0)
			{
			...
			else
			{
				...
				while(true)
				{
					if(compare(temp->getData(), val) == 1)//if(temp->getData() > val)
					{
					...
					else if(compare(temp->getData(), val) == -1)//else if(temp->getData() < val)
					{
