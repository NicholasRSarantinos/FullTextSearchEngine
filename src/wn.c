...
/////
char **write_buffer;
unsigned int *max_write_buffer_size;
unsigned int *current_pos;

void set(char **write_buffer_in, unsigned int *max_write_buffer_size_in, unsigned int *current_pos_in)
{
	write_buffer = write_buffer_in;
	max_write_buffer_size = max_write_buffer_size_in;
...
void add(char *str)
{
	...
	while(str[pos] != 0)
	{
		(*write_buffer)[((*current_pos)++)] = str[pos];

		if(*current_pos-2 == *max_write_buffer_size)
		{
			printf("%s", "NOT EXPECTED IN WORDNET ISSUE!!\n");
			getchar();
		}
...
{
	set(write_buffer, max_write_buffer_size, current_pos);
   ...
   (*write_buffer)[((*current_pos)++)] = 0;//to finish it
}
